...
;Problem: 4
; Add let to TLS, and prove that the resulting interpreter (for the language TLS-let, that is, TLS with let) is correct.
...
(define *let
  (lambda (e table)
    (build (quote non-primitive)
           (cons table (cons (cons (firsts (formals-of e))
                                   (cons(body-of e) '()))
                             (seconds (formals-of e)))))))
...
;design *let that works same as the built in let
;of R5RS scheme

;if
(define my-let '(let ((x 1) (y 2)) (+ x y)))
 
;is equivalent to
;((lambda (x y) (+ x y)) 1 2)

;we can then change a let expression into a lambda expression then
;which we know evaluates correctly in our tls interpreter

;to change a let expression into a lambda expression, our program does the
;following...

;we use helper functions to get the parts of the let expression
;and cons them together so we get a lambda expression from the let
;expression

;firsts of formals-of of let-exp: formals of lambda expression
;seconds of formals-of of let-exp: values of lambda expression
;body-of let expression: body of lambda expression
